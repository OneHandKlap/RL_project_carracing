    ...
    def initialize_models(self):
        ...
        def create_dqn_model(input_channel, out_channels, filter_sizes, strides, paddings):
            ...
            model = nn.Sequential(OrderedDict([
                ('0', nn.Conv2d(input_channel, out_channels[0], filter_sizes[0], stride=strides[0], padding=paddings[0])),
                ('1', nn.ReLU()),
                ('2', nn.Conv2d(out_channels[0], out_channels[1], filter_sizes[1], stride=strides[1], padding=paddings[1])),
                ('3', nn.ReLU()),
                ('4', nn.Conv2d(out_channels[1], out_channels[2], filter_sizes[2], stride=strides[2], padding=paddings[2])),
                ('5', nn.ReLU()),
                ('6', nn.Flatten()),
                ('7', nn.Linear(out_channels[3], out_channels[4])),
                ('8', nn.ReLU()),
                ('9', nn.Linear(out_channels[4], out_channels[5]))
            ]))
            #print(model)
            return model

    ...
    def test(self, num_episodes=50, epoch=1):
        ...
        for episode in range(num_episodes):
            ...
            for i in range(10000):
                action = self.select_deterministic_action(torch.flatten(rn18_fe.forward(state), 1))
...
model = RL_Model(cp, DQN, action_space=[0,1])
#model.load("rl_cartpole_ep_50.pth")
#print(model.test())

# %%
for i in range(1, 2):
    ep, rewards = model.train(
        5000, render=False, epoch=i)
    model.save("rl_cartpole_ep_" + str(i * 50))
    model.generate_policy_video("rl_cartpole_ep_" + str(i*50))
plt.savefig("rl_cartpole_plt.png")

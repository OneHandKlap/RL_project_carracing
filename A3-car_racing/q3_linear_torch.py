...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        state_flattened = torch.flatten(state, start_dim=1, end_dim=-1)
        #print('flattened.shape =', state_flattened.shape)
        if network == 'q_network':
            out = self.q_network(state_flattened)
            #print('out.shape =', out.shape)
        else:
            out = self.target_network(state_flattened)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        target_q_max, _ = torch.max(target_q_values, 1)
        Q_samp = torch.mul(rewards, done_mask) +                     (torch.mul(gamma * target_q_max, 1 - done_mask) + torch.mul(rewards, 1 - done_mask))
        Q = torch.sum(torch.mul(q_values, F.one_hot(actions, num_actions)), 1)
        loss = torch.sum(F.mse_loss(Q, Q_samp, reduction='mean'))
        ##############################################################
        ######################## END YOUR CODE #######################
        return loss
